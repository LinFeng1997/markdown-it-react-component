...
function pushStack(stack,lineText) {
  ...
  for (; i < REACT_SEQUENCES.length; i++) {
    ...
    if (match && typeof REACT_SEQUENCES[i][3] === 'function') {
      openTag = match[1];
      const tagMatch = REACT_SEQUENCES[i][3](openTag);
...
export default function react_block(state, startLine, endLine, silent) {
  ...
  let endReg = REACT_SEQUENCES[i][1];
  if (typeof REACT_SEQUENCES[i][3] === 'function'){
    endReg = REACT_SEQUENCES[i][3](openTag);
  }
  ...
  if (!endReg.test(lineText)) {
    ...
    for (; nextLine < endLine; nextLine++) {
      ...
      if(innerTag && typeof REACT_SEQUENCES[i][3] === 'function'){
        innerEndReg = REACT_SEQUENCES[i][3](innerTag);
      }
      if(stack.length === 1){
        innerEndReg = endReg;
      }
