...
// export const REACT_TAG_RE = new RegExp('^(?:' + open_tag + '|' + close_tag + ')');

const REACT_OPEN_CLOSE_TAG_RE = /^<([A-Z][A-Za-z0-9]*).*<\/\1>/;
// const REACT_TAG_RE = /^<([A-Z][A-Za-z0-9]*).*<\/\1>/;
...
export function react_inline(state, silent) {
  ...
  let match = state.src.slice(pos).match(REACT_OPEN_CLOSE_TAG_RE);
